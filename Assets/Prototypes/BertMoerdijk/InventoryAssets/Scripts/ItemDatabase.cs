...
public class ItemDatabase : MonoBehaviour {
	...
	void Start () 
	{
		...
		itemData = JsonMapper.ToObject(File.ReadAllText(Application.dataPath + "/Prototypes/BertMoerdijk/InventoryAssets/StreamingAssets/Items.json"));
    ...
    void ConstructItemDatabase ()
    {
        for (int i = 0; i < itemData.Count; i++)
        {
            dataBase.Add(new Item((int)itemData[i]["id"], (string)itemData[i]["title"],
                ...
                itemData[i]["slug"].ToString(), itemData[i]["animation"].ToString()));
        }
    }
}
...
public class Item
{
    ...
    public Item(int id, string title, int damage, int range, string description, bool stackable, string itemtype, string slug, string animation)
    {
        this.ID = id;
        this.Title = title;
        this.Damage = damage;
